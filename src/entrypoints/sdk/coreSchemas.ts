...
export const AccountInfoSchema = lazySchema(() =>
  z
    .object({
      ...
      apiProvider: z
        .enum(['firstParty', 'bedrock', 'vertex', 'foundry', 'qwen'])
        .optional()
        .describe(
          'Active API backend. Anthropic OAuth login only applies when "firstParty"; for 3P providers the other fields are absent and auth is external (AWS creds, gcloud ADC, Qwen API key, etc.).',
        ),
    })
